...
const DailyEntryForm: React.FC<DailyEntryFormProps> = ({ foodItems, onSubmit, dailyEntries, entryToEdit }) => {
    ...
    const calculatedScoreDisplay = useMemo(() => {
        ...
        tempEntries.sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    
        const tempIndex = tempEntries.findIndex(e => e.date === date);
        const tempPrev = (tempIndex > 0 && isNextDay(tempEntries[tempIndex-1].date, tempEntries[tempIndex].date)) ? tempEntries[tempIndex-1] : null;
        const tempNext = (tempIndex < tempEntries.length - 1 && isNextDay(tempEntries[tempIndex].date, tempEntries[tempIndex+1].date)) ? tempEntries[tempIndex+1] : null;

        const { score, calculation } = calculateDailyScore(currentEntry, tempPrev, tempNext);
            ...
            <div className="p-6 bg-white rounded-xl shadow-lg">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <div className="food-checkbox-list mt-1 max-h-40 overflow-y-auto border border-gray-300 rounded-md p-2 bg-white">
